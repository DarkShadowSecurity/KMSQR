...
from .oidc import OidcError, make_pkce
from .sessions import SESSION_COOKIE, OIDC_TX_COOKIE
...
def build_router(ks: KeyStore, audit: AuditLog, auth: TokenAuth, authz: Authorizer, limiter: Limiter,
                 *, sessions=None, oidc=None, cookie_secure: bool = True) -> APIRouter:
    r = APIRouter(prefix="/api/v1")
    sso_on = bool(oidc is not None and getattr(oidc, "config", None) and oidc.config.enabled and sessions is not None)

    def scope(required: str):
        # Authorize via bearer token or (if SSO is on) a session cookie.
        return require_scope(auth, required, sessions)
    ...
    def status(request: Request, caller=Depends(scope(SCOPES_READ))):
    ...
    def create_key(request: Request, req: CreateKeyReq, caller=Depends(scope(SCOPES_MANAGE))):
    ...
    def list_keys(request: Request, page: tuple = Depends(_pagination),
                  caller=Depends(scope(SCOPES_READ))):
        return authz.filter_visible_keys(caller, ks.list_keys(limit=page[0], offset=page[1]))

    @r.get("/keys/{key_id}")
    @limiter.limit("120/minute")
    def get_key(request: Request, key_id: str, caller=Depends(scope(SCOPES_READ))):
    ...
    def rotate(request: Request, key_id: str, caller=Depends(scope(SCOPES_MANAGE))):
    ...
    @r.post("/keys/import")
    @limiter.limit("30/minute")
    def import_key(request: Request, req: ImportKeyReq, caller=Depends(scope(SCOPES_MANAGE))):
        namespace_id = _namespace_id_for(req.namespace)
        authz.authorize_namespace_op(caller, OP_MANAGE, namespace_id)
    ...
    @limiter.limit("120/minute")
    def public_key(request: Request, key_id: str, caller=Depends(scope(SCOPES_READ))):
        authz.authorize_key_op(caller, OP_READ, key_id)
        return _safe_call("key.public_key", ks.export_public_key, key_id)

    @r.post("/keys/{key_id}/disable")
    @limiter.limit("30/minute")
    def disable_key(request: Request, key_id: str, caller=Depends(scope(SCOPES_MANAGE))):
        authz.authorize_key_op(caller, OP_MANAGE, key_id)
    ...
    @limiter.limit("30/minute")
    def enable_key(request: Request, key_id: str, caller=Depends(scope(SCOPES_MANAGE))):
        authz.authorize_key_op(caller, OP_MANAGE, key_id)
    ...
    @limiter.limit("30/minute")
    def cancel_deletion(request: Request, key_id: str, caller=Depends(scope(SCOPES_MANAGE))):
        authz.authorize_key_op(caller, OP_MANAGE, key_id)
    ...
    def encrypt(request: Request, key_id: str, req: EncryptReq, caller=Depends(scope(SCOPES_ENCRYPT))):
    ...
    @limiter.limit("600/minute")
    def decrypt(request: Request, key_id: str, req: DecryptReq, caller=Depends(scope(SCOPES_DECRYPT))):
    ...
    @limiter.limit("600/minute")
    def sign(request: Request, key_id: str, req: SignReq, caller=Depends(scope(SCOPES_SIGN))):
    ...
    @limiter.limit("600/minute")
    def verify(request: Request, key_id: str, req: VerifyReq, caller=Depends(scope(SCOPES_VERIFY))):
    ...
    @limiter.limit("600/minute")
    def wrap(request: Request, key_id: str, req: WrapReq, caller=Depends(scope(SCOPES_ENCRYPT))):
        authz.authorize_key_op(caller, OP_ENCRYPT, key_id)
    ...
    @limiter.limit("600/minute")
    def unwrap(request: Request, key_id: str, req: UnwrapReq, caller=Depends(scope(SCOPES_DECRYPT))):
        authz.authorize_key_op(caller, OP_DECRYPT, key_id)
        try:
    ...
    @r.get("/audit")
    @limiter.limit("60/minute")
    def audit_list(request: Request, limit: int = 200, caller=Depends(scope(SCOPES_READ))):
        if limit < 1 or limit > 1000:
    ...
    @r.get("/audit/verify")
    @limiter.limit("10/minute")
    def audit_verify(request: Request, caller=Depends(scope(SCOPES_ADMIN))):
    ...
    @r.post("/namespaces")
    @limiter.limit("10/minute")
    def create_namespace(request: Request, req: CreateNamespaceReq, caller=Depends(scope(SCOPES_ADMIN))):
    ...
    @r.get("/namespaces")
    @limiter.limit("60/minute")
    def list_namespaces(request: Request, caller=Depends(scope(SCOPES_READ))):
        return ks.repo.list_namespaces()  # bounded set; pagination not needed

    # ---- grants ----
    @r.post("/grants")
    @limiter.limit("30/minute")
    def create_grant(request: Request, req: CreateGrantReq, caller=Depends(scope(SCOPES_ADMIN))):
        import uuid as _uuid
        from datetime import datetime as _dt, timezone as _tz
        try:
    ...
    def list_grants(request: Request, principal_id: Optional[str] = None, page: tuple = Depends(_pagination),
                    caller=Depends(scope(SCOPES_ADMIN))):
    ...
    @limiter.limit("60/minute")
    def delete_grant(request: Request, grant_id: str, caller=Depends(scope(SCOPES_ADMIN))):
    ...
    @r.post("/principals")
    @limiter.limit("10/minute")
    def create_principal(request: Request, req: CreatePrincipalReq, caller=Depends(scope(SCOPES_ADMIN))):
        try:
    ...
    @limiter.limit("60/minute")
    def delete_principal(request: Request, principal_id: str, caller=Depends(scope(SCOPES_ADMIN))):
    ...
    @r.post("/tokens")
    @limiter.limit("10/minute")
    def create_token(request: Request, req: CreateTokenReq, caller=Depends(scope(SCOPES_ADMIN))):
        try:
    ...
    @limiter.limit("60/minute")
    def revoke_token(request: Request, token_id: str, caller=Depends(scope(SCOPES_ADMIN))):
    ...
    def auth_config(request: Request):
        """Unauthenticated: lets the UI decide whether to show an SSO login button."""
        return {"sso_enabled": sso_on}

    if sso_on:
        _OIDC_SUB_PREFIX = "oidc_sub:"  # meta key prefix for IdP-subject -> principal mapping
        ...
        def auth_login(request: Request):
            ...
            try:
                ...
                log.exception("oidc authorization_url failed")
                raise HTTPException(502, "identity provider unavailable")
            tx = sessions.sign_tx({"state": state, "nonce": nonce, "verifier": verifier})
            resp = RedirectResponse(url, status_code=302)
            resp.set_cookie(OIDC_TX_COOKIE, tx, max_age=600, httponly=True,
        ...
        def auth_callback(request: Request, code: str = "", state: str = ""):
            ...
            tx = sessions.verify_tx(tx_cookie) if tx_cookie else None
            if not tx or not code or not state or not _secrets.compare_digest(state, tx.get("state", "")):
            ...
            try:
                ...
                claims = oidc.verify_id_token(id_token, tx["nonce"])
            except OidcError as e:
                log.warning("oidc login failed: %s", e)
            ...
            sub = claims.get("sub")
            if not sub:
                raise HTTPException(401, "id_token missing subject")
            display = claims.get("email") or claims.get("preferred_username") or sub

            # Map the IdP subject to a stable human principal (persisted in meta).
            meta_key = _OIDC_SUB_PREFIX + sub
            existing = ks.repo.get_meta(meta_key)
            pid = existing.decode() if existing else None
            if pid is None or auth.get_principal(pid) is None:
                pid = auth.create_principal(display, ptype="human")
                ks.repo.set_meta(meta_key, pid.encode())

            user_scopes = oidc.claims_to_scopes(claims)
            cookie_val = sessions.issue(pid, user_scopes, sub)
            audit.append(f"principal:{pid}", "auth.login", target=pid,
                         detail={"sub": sub, "scopes": sorted(user_scopes)})
            resp = RedirectResponse("/ui", status_code=302)
            resp.set_cookie(SESSION_COOKIE, cookie_val, max_age=sessions.ttl_seconds,
        ...
        @r.get("/auth/me")
        @limiter.limit("120/minute")
        def auth_me(request: Request, caller=Depends(require_auth(auth, sessions))):
            return {"principal_id": caller.principal_id, "display_name": caller.display_name,
